...
(function($) {
    $.fn.graspSchedule = function(paramData) {
        ...
        var checkParamDate = function() {
            ...
            if (typeof paramData.items === "undefined" && typeof paramData.schedules === "undefined") {
                return false;
            }
            if (typeof paramData.items === "undefined") {
                paramData.items = [];
            }
        ...
        var getEventHight = function(eventId) {
            if (paramData.items[eventId].css && paramData.items[eventId].css.height) {
                return extractNum(paramData.items[eventId].css.height);
            }
            ...
            jqueryThis.append("<div class='" + paramData.options.classnames.event + "' id='" + id2 + "'>" + paramData.items[eventId].title + "<br>" + paramData.items[eventId].date + "</div>");
        ...
        var calculateTime = function() {
            ...
            for (i2 = 0; i2 < paramData.items.length; i2++) {
                timeList.push({time: paramData.items[i2].date, kind: 'event', id: i2});
            }
            timeList.sort(
                    function(a, b) {
        ...
        var drawBlockSchedule = function() {
            ...
            var drawEvent = function(id) {
                ...
                jqueryThis.append("<div class='" + paramData.options.classnames.event + "' id='" + id2 + "'>" + paramData.items[id].title + "<br>" + comDateFormat(paramData.items[id].date) + "</div>");
                var css = {};
                if (paramData.items[id].css) {
                    css = $.extend(true, {}, paramData.items[id].css);
